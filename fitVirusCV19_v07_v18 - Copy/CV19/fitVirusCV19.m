function [res,t] = fitVirusCV19(getData,varargin)
...
%         disp(Ca/sf)
        ...
        if ~fdata
            ...
            scatter(ttd, C00/sf,50,'k','filled')
       ...
       % xlim([t(1),t(end)]+date0);
        ...
        %... add title
        tx1 = sprintf('%s : coronavirus epidemic (SIR model): %s',...
        ...
        bar( date0 + ttt + 1,dC)
          ...
          bar( date00 + (1:length(C00)-1),diff(C00))
        end
        ttd = t(2:end) + date0;
        ...
        %... add title
        title('Daily New Cases','FontWeight','normal')
        
        % ... add labels
        ylabel('Novel cases / day')
        xlabel('Date')
        ...
        if nsp == 3
            ...
            %... plot actual
            ttd = tt(2:end)+date0;
            scatter(ttd,(dC./C(1:end-1))*100,50,'k','filled')
            ...
            %... add title
            title('Daily Cases Growth Factor','FontWeight','normal');
